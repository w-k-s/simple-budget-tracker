...
func simulateRecords(db *sql.DB, numberOfUsers int, startMonth ledger.CalendarMonth, endMonth ledger.CalendarMonth) ([]ledger.AccountId, error) {
	...
	for i := 0; i < numberOfUsers; i++ {
		...
		if currentAccount, err = ledger.NewAccount(ledger.AccountId(time.Now().UnixNano()), "Current", "AED"); err != nil {
			return nil, err
		}
		if savingsAccount, err = ledger.NewAccount(ledger.AccountId(time.Now().UnixNano()), "Savings", "AED"); err != nil {
			return nil, err
		}
		...
		if salaryCategory, err = ledger.NewCategory(ledger.CategoryId(time.Now().UnixNano()), "Salary"); err != nil {
			return nil, err
		}
		if savingsCaegory, err = ledger.NewCategory(ledger.CategoryId(time.Now().UnixNano()), "Savings"); err != nil {
			return nil, err
		}
		if billsCategory, err = ledger.NewCategory(ledger.CategoryId(time.Now().UnixNano()), "Bills"); err != nil {
			return nil, err
		}
		if foodAndDrinkCategory, err = ledger.NewCategory(ledger.CategoryId(time.Now().UnixNano()), "Food & Drink"); err != nil {
			return nil, err
		}
		...
		for date := fromDate; date.Before(toDate); date = date.AddDate(0, 0, 1) {
			if date.Day() == 1 {
				...
				// Add salary
				if salaryRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Salary", salaryCategory, quickMoney("AED", 50_000_00), date, ledger.Income, 0); err != nil {
					return nil, err
				}
				// Pay bills
				if billsRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Bills", billsCategory, quickMoney("AED", 1_000_00), date, ledger.Expense, 0); err != nil {
					return nil, err
				}
				// Save a little money for a rainy day
				if savingsRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Savings", savingsCaegory, quickMoney("AED", 10_000_00), date, ledger.Transfer, savingsAccount.Id()); err != nil {
					return nil, err
				}
			...
			if lunchRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Lunch", foodAndDrinkCategory, quickMoney("AED", 10_00), date, ledger.Expense, 0); err != nil {
				return nil, err
			}
			if dinnerRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Dinner", foodAndDrinkCategory, quickMoney("AED", 20_00), date, ledger.Expense, 0); err != nil {
				return nil, err
			}
			...
			if date.Day() == 2 && date.Month() == time.June {
				...
				if specialOccasionRecord, err = ledger.NewRecord(ledger.RecordId(time.Now().UnixNano()), "Birthday", foodAndDrinkCategory, quickMoney("AED", 100_00), date, ledger.Expense, 0); err != nil {
					return nil, err
				}
